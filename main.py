...
def new_bar_event_pairs(name_1, name_2, last_bar_1, last_bar_2):
    ...
    date_str_1 = last_bar_1['time'].iloc[0]
    date_format = '%Y-%m-%d %H:%M:%S'
    # date_format = '%Y-%m-%d'

    last_bar_time_1_formatted = datetime.strptime(date_str_1, date_format)

    # Fixing a formatting error when getting last_bar['time'].iloc[0]
    date_str_2 = last_bar_2['time'].iloc[0]
    date_format = '%Y-%m-%d %H:%M:%S'
    # date_format = '%Y-%m-%d'

    last_bar_time_2_formatted = datetime.strptime(date_str_2, date_format)
    ...
    while True:
        ...
        if (new_bar_1['time'].iloc[0] != last_bar_time_1_formatted) or new_bar_2['time'].iloc[
            0] != last_bar_time_2_formatted:
            print(f'{name_1}, {name_2}: new tick: {datetime.now()}')
            return new_bar_1, new_bar_2

        # s_print(f'{name_1}, {name_2}: tick: {datetime.now()}')
        time.sleep(5)
...
def backtest(name_1, name_2, data_close):
    ...
    rolling_mean = data_close.rolling(rolling_interval).mean()
    rolling_std = data_close.rolling(rolling_interval).std()

    tp = rolling_mean
    open_high = rolling_mean + (2 * rolling_std)
    open_low = rolling_mean - (2 * rolling_std)

    sl_high = rolling_mean + (3 * rolling_std)
    sl_low = rolling_mean - (3 * rolling_std)
    ...
    for t in range(1, len(data_close)):
        ...
        if position_type == FREE:
            if data_close.iloc[t - 1] < open_high.iloc[t - 1] and data_close.iloc[t] >= open_high.iloc[t]:
                # Short first Long last
                entries[t] = True
                position_type = HIGH

            elif data_close.iloc[-2] > open_low.iloc[t - 1] and data_close.iloc[t] <= open_low.iloc[t]:
                # Long first Short last
                entries[t] = True
                position_type = LOW

        elif position_type == HIGH:
            if data_close.iloc[t - 1] > tp.iloc[t - 1] and data_close.iloc[t] <= tp.iloc[t]:
                exits[t] = True
                position_type = FREE
            elif data_close.iloc[t - 1] < sl_high.iloc[t - 1] and data_close.iloc[t] >= sl_high.iloc[t]:
                exits[t] = True
                position_type = FREE
            ...
            if data_close.iloc[t - 1] < tp.iloc[t - 1] and data_close.iloc[t] >= tp.iloc[t]:
                exits[t] = True
                position_type = FREE
            elif data_close.iloc[t - 1] > sl_low.iloc[t - 1] and data_close.iloc[t] <= sl_low.iloc[t]:
                exits[t] = True
                position_type = FREE
...
def trade_engine(data, name_1, name_2, lot_size_1, lot_size_2, position_type, long_ticket, short_ticket):
    ...
    # rolling_mean = data.rolling(rolling_interval).mean()
    # rolling_std = data.rolling(rolling_interval).std()

    mean = data.mean()
    std = data.std()

    tp = mean
    open_high = mean + (1.5 * std)
    open_low = mean - (1.5 * std)
...
def pairs_trader(name_1, name_2, slope, risk):
    end_date = datetime.now() + timedelta(hours=3)
    start_date = end_date - BDay(10)

    print(f'Process: {name_1}, {name_2} started.')

    # Gather the stock data and make the artificial asset
    data_1 = get_bars_from_broker(name_1, start_date, end_date)
    data_2 = get_bars_from_broker(name_2, start_date, end_date)

    # Save all three data to .csv's for later
    aa = 'artificial_asset'

    hash_1 = hashlib.md5((name_1 + name_2).encode())
    hash_2 = hashlib.md5((name_2 + name_1).encode())
    hash_aa = hashlib.md5((aa + name_1 + name_2).encode())

    data_1.to_csv('trade_engine_data/' + name_1 + hash_1.hexdigest() + '.csv', index=False)
    data_2.to_csv('trade_engine_data/' + name_2 + hash_2.hexdigest() + '.csv', index=False)

    data_1 = pd.read_csv('trade_engine_data/' + name_1 + hash_1.hexdigest() + '.csv')
    data_2 = pd.read_csv('trade_engine_data/' + name_2 + hash_2.hexdigest() + '.csv')

    data_1_close = data_1['close']
    data_2_close = data_2['close']

    if data_1_close.size > data_2_close.size:
        data_1_close = data_1_close.iloc[data_1_close.size - data_2_close.size:].reset_index(drop=True)
    elif data_2_close.size > data_1_close.size:
        data_2_close = data_2_close.iloc[data_2_close.size - data_1_close.size:].reset_index(drop=True)

    artificial_asset = data_1_close + slope * data_2_close

    artificial_asset.to_csv('trade_engine_data/' + aa + hash_aa.hexdigest() + '.csv', index=False)

    aa_data = pd.read_csv('trade_engine_data/' + aa + hash_aa.hexdigest() + '.csv')
    ...
    draw_graph(
        ...
        artificial_asset.mean() + 1.5 * artificial_asset.std(),
        artificial_asset.mean() - 1.5 * artificial_asset.std(),
    ...
    while True:
        ...
        aa_new_bar = new_bar_1['close'] + slope * new_bar_2['close']

        new_bar_1.to_csv('trade_engine_data/' + name_1 + hash_1.hexdigest() + '.csv', mode='a', index=False,
                         header=False)
        new_bar_2.to_csv('trade_engine_data/' + name_2 + hash_2.hexdigest() + '.csv', mode='a', index=False,
                         header=False)
        aa_new_bar.to_csv('trade_engine_data/' + aa + hash_aa.hexdigest() + '.csv', mode='a', index=False, header=False)
...
if __name__ == '__main__':
    ...
    for data in tqdm(consistent_pairs_close_data):
        ...
        data_2_close = data['close_data']['close0']
        ...
        try:
            ...
            # print(f'{name_1}, {name_2}: VALUE ERROR')
            continue
        ...
        if 1 < pf.sharpe_ratio() < math.inf:
            ...
            # print(f'{name_1}, {name_2}, SR: {pf.sharpe_ratio()}, TP: {pf.total_profit()}')
    ...
    for pair in profitable_pairs:
        ...
        trader = Process(target=pairs_trader, args=(name_1, name_2, slope, risk))
        trader.start()
        time.sleep(2)
